...
### 3.2b 源更新时间（Hero 醒目标签）

| 页面信息 | 模板变量 | 表.字段 | 写入 | 生成 |
|----------|----------|---------|------|------|
| 源更新时间（Hero badge） | `magnets_updated_at` / `_display` / `_date` | `MAX(download_resources.indexed_at)`；无则回退 `media_pages.updated_at` | `upsert_slot_resources` 写 `indexed_at` | `resolve_magnets_updated_fields` → episode/movie Hero `rm-badge--updated`；Recommended 区标题旁亦可展示 |
| 首页卡片更新日 | 同上（条目级） | 同 catalog 下 `MAX(indexed_at)` | 同上 | `list_home_catalog_entries` **按该字段降序** → `home.html` 卡片 |
...
| 测速区域 | `speed_evidence.test_region_label` | `slot_speed_summary.test_region`（缺省回退 `RM_SPEEDTEST_REGION`） | 同上 | `speedtest.region` + `speed_test_facts.html` |
