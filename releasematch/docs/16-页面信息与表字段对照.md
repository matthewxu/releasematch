...
| 页面信息 | 模板变量 | 表.字段 | 写入 | 生成 |
|----------|----------|---------|------|------|
| 源更新时间（Hero badge） | `magnets_updated_at` / `_display` / `_date` | 仅统计 **非空 `magnet_uri`** 行的 `MAX(indexed_at)`；无 magnet → 不展示 | `upsert_slot_resources` 写 `indexed_at` | `resolve_magnets_updated_fields`（**不**回退 `media_pages.updated_at`） |
| 首页卡片更新日 | 同上（条目级） | 同上；收录须实有 ≥2 条 magnet | 同上 + `reconcile_magnet_counts_from_resources` | `list_home_catalog_entries` **按该字段降序**；无更新时间排后且不写卡片时间 |
