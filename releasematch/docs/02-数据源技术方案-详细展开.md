...
| 中文资源 | 私有/半私有 tracker | 公开源覆盖弱 | 见 [11-CN华语影视资源方案](./11-CN华语影视资源方案.md) |
...
### 11.5 跨源 per-indexer 统计（2026-07-05）

> **实现：** `workflow/torrent_sources/cross_source.py` · **登记册：** [IG信息登记册 §五](./IG信息登记册.md#五跨源验证-s-03--s-04计算逻辑与测试结果) · **SEO 迭代：** [跨源扩展与全站重拉](./seo/iterations/2026-07-05-跨源扩展与全站重拉.md)

**源族键（`INDEXER_LABEL`）不再把全部 Jackett indexer 折叠为单一 `jackett`：**

| 源 | 族键示例 | 计入分母条件 |
|----|----------|--------------|
| EZTV API | `eztv` | `source_enabled` 且路由命中 |
| Nyaa RSS | `nyaa` | 同上 |
| YTS API | `yts` | 电影路由 |
| Jackett indexer | `jackett:thepiratebay`、`jackett:1337x`、`jackett:torrentgalaxyclone`、`jackett:nyaasi` … | `accounts.local.json` → `jackett.indexers` 中启用且 `fetch_service` 展开 |

**默认分母（`cross_source_total`）：**

| 媒体类型 | 默认分母 | 说明 |
|----------|----------|------|
| 剧集 | **4** | eztv + nyaa + 2 个 Jackett indexer（按路由） |
| 电影 | **3** | yts + Jackett indexer 组合 |

**分子（`cross_source_count`）：** 同一 infohash 在 ≥2 个**不同源族**出现时 +1；fuzzy 对齐见 `scripts/recompute_cross_source_fuzzy.py`（不重拉 indexer）。
...
**2026-07-05 全站重拉后基线：** 分母均值 **3.28**（max **6**）· 分子 cross≥2 仍 **3/117**（质量向待 fuzzy）。
...
| **DMHy RSS** | 华语动漫/部分剧集 | RSS keyword | 无 | ⭐⭐⭐⭐ | **P0**（[11 文档](./11-CN华语影视资源方案.md)） |
...
| v1.3 | 2026-07-05 | §11.5 跨源 per-indexer 统计；refetch-all / fuzzy 运维顺序 |
