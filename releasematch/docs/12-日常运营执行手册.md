...
> **Ops UI：** `python -m workflow.run ops serve` → **⑥ 日常运营**（§四巡检一键验收）  
> **排障：** 各按钮旁 **?** → 实现流程 / 脚本 / CLI / 数据落点（`workflow/ops/static/ops-help.js`）  
...
| [16-页面信息与表字段对照.md](./16-页面信息与表字段对照.md) | 页面区块 ↔ 表字段 ↔ 写入脚本 ↔ 生成模块 | 改模板/加字段时 |
...
| [15-多地多环境开发切换.md](./15-多地多环境开发切换.md) | 两地 / Debian 12·13 开发机：私有仓同步、§三-B apt | 切换流程变更时 |
| [17-运维机迁移到服务器.md](./17-运维机迁移到服务器.md) | 运维机 → `8.159.152.227` step-by-step（Ops / cron） | 迁运维机时 |
...
| **sitemap** | **37 URL**（首页 + Trust **6** + ≤30 indexable） | D3 决策；Hub **noindex,follow** |
| **C2 本地门禁** | **`seo_c2_checklist` 13 pass / 0 fail** | OG + favicon + Schema 已落地（2026-07-05） |
| **测速 cron** | 每 6h · `--all-published --write --workers 5` | Rec+summary **110/110**（2026-07-21 巡检） |
| **增量发布 cron** | 每 6h+30m · `incremental_publish_worker` | 脏页 → bake dist；CF 暂缓阶段 `--prepare-only` |
| **VPS** | `104.105.140.95` | Jackett + FlareSolverr + SSH SOCKS |
...
| **Ops 日常入口** | `ops serve` → **⑥ 日常运营** | `GET /api/daily/status` 一键巡检 |
...
**数据流口诀：** Jackett 拉 magnet → scorer 选 Recommended → MySQL 定 `page_status` → 测速写 `slot_speed_summary` / `torrent_metadata` → Jinja bake 进 dist → SEO 门禁 → wrangler 上线；上线后靠测速 cron + **增量发布 cron** 保持 dist 与库一致。
...
- Hub 页（`/breaking-bad/`）**noindex,follow**（D2），不进 sitemap；`ensure_slot_page` / Ops generate 会同步 `tv:{tmdb}:hub`，避免 Hub 404
- 剧集 Recommended 与 All Sources 共用 `rm-table--episode` + `rm-col-*` 固定列宽（无 ellipsis；表宽 100%）
- **TMDB 海报/简介**：建槽时写入 MySQL；日常更新 magnet / 测速不必再拉 TMDB。存量：`python -m workflow.run meta enrich --all-empty`
...
**逻辑：** `deploy_cf_pages.sh` 默认 = `generate all` → 同步 static/404/410 → `wrangler deploy`。  
另支持 `--mode incremental --page-ids …`（只 bake 指定槽 + home/sitemap）与 `--mode upload-only`（只上传当前 dist）。Ops ④ 上线 UI 同源。wrangler 按 hash 增量上传；dist 缺路径则对账删除。
...
| dist 无 metadata 面板 | 测速后未 `generate` | CLI：`generate page`；**Ops 测速 write 已自动 regenerate** |
| 剧集 `/{slug}/` 404 | 无 Hub 行 | `ensure_show_hub_page` 后 `generate page --page-id tv:…:hub`（Ops generate 已自动） |
| 侧栏无海报/简介 | catalog 未 enrich | `meta enrich --page-id …` 或 `--all-empty`（仅首次；日常不必） |
...
| **数据源（VPS）** | Jackett / FlareSolverr / SOCKS | 无业务库 | — | `104.105.140.95:9117` |
...
```bash
python -m workflow.run ops serve   # http://127.0.0.1:8090/ → ⑤ 配置 →「一键部署 Jackett + FlareSolverr」
# 可「从 servers.local.json 预填」；密码留空则用本地凭据；勾选 indexer + 同步 Key
```
...
# 交互一键（IP + 密码；可询问是否写入默认 indexer）
# Dashboard 登录密码默认 345621（可用 JACKETT_ADMIN_PASSWORD 覆盖）
...
JACKETT_ADMIN_PASSWORD='345621' bash scripts/install_jackett_oneclick.sh --host 104.105.140.95 --password 'YourPass' --with-indexers
...
详见 [VPS迁移与部署.md](./VPS迁移与部署.md) · [06 §5.4b](./06-run-cli使用说明.md)。
...
| `RM_SITE_ORIGIN` | `.env` 中 canonical / OG 域名（**apex** `https://releasematch.com`，非 www） |
| **www 301** | Cloudflare Page Rule `www.releasematch.com/*` → `https://releasematch.com/$1`（见 [infra-www-to-apex-301.md](./seo/infra-www-to-apex-301.md)） |
...
[ ] www → apex 301（公网 curl 或 `seo_c2_checklist.py --check-live-www`）
```
...
# 每 6h+30m — 脏页检测 → 增量 bake dist（CF 正式上线暂缓时用 --prepare-only；上线后去掉该旗标即 wrangler 上传）
...
| **每 6h+30m（自动）** | 脏页检测 → 增量 bake（可选 deploy） | §五附 | 0（cron） |
| **每日** | 数据源 status · DB 行数 · **TMDB 日同步** · cron 日志 | §四 · Ops ⑥ | ~10 min |
...
flowchart LR
    subgraph auto [自动 每6h 增量]
        ...
        PUB[incremental_publish_worker]
        CRON --> PUB
    end
    subgraph daily [每日人工 / Ops⑥]
        ST[torrent status]
        DB[db status]
        TMDB[ops tmdb-sync]
        LOG[cron 日志]
...
> **推荐 UI：** `python -m workflow.run ops serve` → **⑥ 日常运营** →「刷新巡检」  
> （`GET /api/daily/status` 汇总本节各项；可一键 TMDB 日同步 / 测速缺口补测）
...
| `slot_speed_summary` | **有 Rec 的 published = summary**（2026-07-21：**110/110**） | 有 Rec 页缺 summary → Ops ⑥「测速缺口补测」或查 cron |
...
> **本机 Ops：** 若无 `/var/log/releasematch`，⑥ 面板会回退展示 `worklogs/**/speedtest*.json` 最近报告；VPS 日志仍须 SSH 查看。

### 4.4 TMDB 标题库日同步

```bash
# CLI（与 cron 06:30 同源；同日幂等可跳过）
python -m workflow.run ops tmdb-sync

# Ops UI：⑥「TMDB 日同步」或 ①「日同步入库」
```

| 通过标准 | 说明 |
|----------|------|
| `tmdb_export_titles` meta `status=ready` | Ops ① 搜索选槽依赖此表 |
| `export_date` 距今 ≤ **3** 天 | 日导出通常滞后 1 天；超过 3 天须强制同步 |

### 4.5 每日勾选（复制到 worklog 或日历）

```
[ ] torrent_sources.run status 通过（或 Ops ⑥ 巡检 PASS）
[ ] db status ping.ok
[ ] 测速覆盖：有 Rec 页均有 summary（缺口则补测）
[ ] TMDB 日同步新鲜（export_date ≤3 天；或已跑 ops tmdb-sync）
[ ] 测速 cron 24h 内至少 1 次成功（VPS 日志或 worklogs 报告）
...
> **要点：** 自动测速 **不是** Ops UI 触发，而是本机/VPS 的 **crontab** 调 `scripts/speedtest_batch_worker.py`。  
> Ops ⑥ 只做巡检验收与缺口补测；日常增量仍靠本节 cron。

### 5.1 实现链路

```mermaid
flowchart LR
    CRON["crontab 0 */6"] --> W["speedtest_batch_worker.py"]
    W --> LOAD["load_batch_targets --all-published"]
    LOAD --> SQL["media_pages: published ∧ magnet≥2"]
    SQL --> POOL["ProcessPool workers=5"]
    POOL --> TTL["check_skip_ttl 每槽"]
    TTL -->|skipped_ttl / no_recommended| SKIP[跳过]
    TTL -->|需测| ST["speedtest Recommended\nPhase1+Phase2 256KB"]
    ST --> WRITE["--write → slot_speed_summary\n+ torrent_metadata"]
```

| 层 | 组件 | 职责 |
|----|------|------|
| 调度 | crontab `0 */6 * * *` | 每 6h 拉起一次（与 TTL 对齐） |
| 入口 | `scripts/speedtest_batch_worker.py` | CLI 封装；写 JSON 报告 |
| 编排 | `workflow/torrent_sources/speedtest/batch_service.py` | 目标加载、TTL、进程池 |
| 测速 | `speedtest_recommended_slot` | 只测该槽 **Recommended** magnet |
| 持久化 | `--write` | `slot_speed_summary` + Phase1/2 明细 + `torrent_metadata` |

### 5.2 清单怎么选定（两层）

**第一层 — 候选清单（`--all-published`）**

等价于 `MySQLStore.list_published_page_ids()`：
...
- 随库内 published 页自动增减，**不必**手维护 JSON
- 含 episode / movie；Hub 若也满足条件会进入（通常 Hub 无 magnet，不会进）
- **不要求**此时已有 Recommended（无 Rec 在第二层跳过）

也可改用：`--page-ids a,b`（针对性）或 `--slots-json …`（benchmark 文件）。

**第二层 — 进候选后是否真测（`check_skip_ttl`，默认 TTL=6h）**

对每个 `page_id`：

| 条件 | 结果 | `status` / reason |
|------|------|-------------------|
| 无 Recommended（`is_recommended=1`） | 不测 | `skipped` / `no_recommended` |
| 无 `slot_speed_summary`，或 `updated_at` 为空 | **要测** | — |
| summary 的 `recommended_infohash` ≠ 当前 Rec infohash | **要测**（Rec 换了） | — |
| 同 hash，且 `updated_at` 距今 **≤ 6h** | 跳过 | `skipped_ttl` / `fresh_within_6h` |
| 同 hash，且已超过 6h | **要测** | — |
| 命令带 `--force` | 全部重测 | 忽略 TTL |
| 带 `--phase2-only` 且仍在 TTL 内 | 仅 Phase2 | `phase2_only`（日常 cron **不用**） |

**口诀：** 候选 = published∧magnet≥2；真测 = 有 Rec 且（无 summary / hash 变了 / 超过 6h）。
...
| 参数 | 值 | 说明 |
|------|-----|------|
| `--all-published` | 必选（日常） | 见 §5.2 第一层 |
| `--workers` | `5` | 与 2026-07-03 benchmark 一致 |
| `--target-bytes` | `262144`（256KB） | 日常默认；重点页可 1MB |
| `--write` | 必选（生产） | 不写库则页面/巡检看不到 summary |
| `--ttl-hours` | 默认 `6` | 与 crontab 周期对齐 |
| torrent metadata | Phase 2 自动写入 | 旧库先 `schema/mysql_migrate_torrent_metadata.sql` |

**测速模式对照：**

| 模式 | 命令要点 | 何时用 |
|------|----------|--------|
| **增量（cron 默认）** | `--all-published`，不加 `--force` | 每 6h；TTL 内 skip |
| **Phase2 增量** | 加 `--phase2-only` | TTL 内仅补 Phase2；日常不必开 |
| **全量强制** | 加 `--force` | 扩槽后 / libtorrent 升级 / benchmark |
| **针对性** | `--page-ids …` 或 `--slots-json` | 单页或少量补测 |
| **Ops 缺口补测** | ⑥「测速缺口补测」或 `POST /api/daily/speedtest-gap` | 有 Rec 缺 summary 时 |
| **不重拉重测** | `speedtest_retest_no_refetch.py` | 回填 metadata / 强制重测 |
...
### 5.4 脏页增量发布 cron（测速写库 → 静态页）

> **要点：** 测速 cron **只写 MySQL**；公网/本地 dist 须再 bake。本 worker 对比 `generated_at` 与子表时间戳，有脏页才增量 `generate` +（可选）`wrangler deploy`。

| 项 | 说明 |
|----|------|
| 调度 | `30 */6 * * *`（测速整点后 30 分钟，错开跑测） |
| 入口 | `scripts/incremental_publish_worker.py` |
| 检测 | `MySQLStore.list_stale_page_ids_for_regenerate`（speed_summary / torrent_metadata / download_resources / catalog / media_pages） |
| bake | `prepare_dist_by_page_ids`（单页 + Hub + home + sitemap + 壳） |
| 上传 | 默认 wrangler；**当前 CF 暂缓**请加 `--prepare-only` |
...
# CF 正式上线后：bake + 上传（去掉 --prepare-only）
python scripts/incremental_publish_worker.py \
  --report /var/log/releasematch/incremental-publish.json
```

**无脏页时** worker 退出 0 并 skip，不浪费 generate / wrangler。

**首轮注意：** 若大量页 `generated_at` 为空，首跑会 bake 很多页；可加 `--limit 30` 分批消化，或先人工 `generate all` 一次对齐水位。
...
**UI 替代路径：** 本地 Ops 控制台可将同一流程拆为「清单从哪来 → 筛选 → 跑生成 → 上线」，筛选后用 MySQL 跟踪表（`ops_track_batches` / `ops_track_slots`）贯通后两步；**日常巡检**走 **⑥ 日常运营**：

```bash
python -m workflow.run ops serve   # http://127.0.0.1:8090/ · 仅本机 · 见 06 §5.4b
# → ⑥ 日常运营：刷新巡检 / TMDB 日同步 / 测速缺口补测
...
**Ops UI：** ③ 跟踪表 `detail` 列（悬停看全文）显示 pipeline/generate/speedtest 最新失败原因；⑥ 日常「失败槽样例」含 `error` 文案。

**「无可用 items」含义与处理：**

Pipeline 在 Jackett/直连拉源后得到 **0 条种子**（或拉取报错）。

- **槽位已有 magnets（≥1）**：不擦库，返回 `kept_existing`（pipeline 标成功），**保留原数据**；Ops 一键流程继续对该槽 **generate + speedtest**（测原 Recommended）。
- **空槽**：仅少数 Demo 槽（如 `tv:1396:s04e06`）回退内置样例；其它空槽才报「无可用 items」。文案里的「db seed」多为历史措辞，**多数情况不是缺库表 seed**。

| 原因 | 是否靠「加更多源」 | 动作 |
|------|-------------------|------|
| Jackett / Indexer / FlareSolverr 故障 | 否（先修连通） | Ops ⑤ Dashboard TEST；`torrent_sources.run status`；已有数据槽可继续测速 |
| 公开源覆盖不足（区域/品类缺口） | **有时有用** | 补 `accounts.local.json` 路由 + VPS indexer 后再 `--force` |
| 冷门 / 成人 / 真稀缺（公开站几乎无种） | **基本没用** | 标 `genuine_scarcity`；已上架槽保留原 magnets |
| 单槽验证 | — | `python -m workflow.torrent_sources.run test --tmdb ID --season 1 --episode 1 --force` |
...
**Ops ③ 一键等价：** 勾选失败槽（或全选）→ 取消「跳过已有 ≥2 magnet」→ **一键跑生成流程**（`POST /api/actions/generation-flow/start`，见 [06-run-cli](./06-run-cli使用说明.md) §5.4b）。
...
| VPS Jackett 一键安装 | Ops ⑤「一键部署 Jackett + FlareSolverr」；或 `bash scripts/install_jackett_oneclick.sh --host 104.105.140.95` |
| VPS Jackett Dashboard 密码 | 默认 `345621`（`JACKETT_ADMIN_PASSWORD`） |
| VPS Jackett 仅装栈 | `bash scripts/deploy_jackett_vps.sh --host 104.105.140.95` |
| VPS Key 同步 | `bash scripts/sync_jackett_vps_key.sh --host 104.105.140.95` |
| **状态** | |
...
| **其他** | |
| TMDB 标题库日同步 | `python -m workflow.run ops tmdb-sync`（或 Ops ⑥） |
| Ops 控制台 | `python -m workflow.run ops serve` → **⑥ 日常运营** / ①–⑤ 扩槽上线 |
| 日常巡检 API | `GET /api/daily/status`（Ops serve 本机） |
...
| Ops ⑥ 样例 | 「刷新巡检」→ 失败槽列表（含 **error**）；登记册路径见 JSON |
| 登记册 | [worklogs/failed-slots-registry.json](../worklogs/failed-slots-registry.json) · `data/failed_slots/registry.json` |
...
- **「无可用 items」**：拉源结果为空（非「缺 DB seed」）。先 `torrent_sources.run test --force` 区分故障 vs 0 命中；**加 indexer 只对覆盖缺口有用，对真稀缺无效**（见 §7.3 表）
...
| [16-页面信息与表字段对照.md](./16-页面信息与表字段对照.md) | 页面区块 ↔ 表字段 ↔ 写入 ↔ 生成 |
| [07-部署架构解疑.md](./07-部署架构解疑.md) | CF Pages · generate · D1 FAQ |
| [VPS迁移与部署.md](./VPS迁移与部署.md) | VPS · cron · systemd |
| [17-运维机迁移到服务器.md](./17-运维机迁移到服务器.md) | 运维机 → `8.159.152.227` 九步迁移 |
...
| v1.2 | 2026-07-05 | 基线 117 indexable · VPS `104.105.140.95` · `pipeline refetch-all` · fuzzy 重算 |
...
| v1.6 | 2026-07-20 | VPS → `104.105.140.95`；§2.4 / 速查改用 `install_jackett_oneclick.sh`（交互 indexer） |
| v1.7 | 2026-07-20 | Dashboard 默认密码 `345621`（`JACKETT_ADMIN_PASSWORD`）；速查补充 |
| v1.8 | 2026-07-21 | §2.4 / 速查：推荐 Ops ⑤ 一键部署 Jackett+FlareSolverr（与 CLI oneclick 同源） |
| v1.9 | 2026-07-21 | §四补齐 TMDB 日同步；测速覆盖基线 110/110；**Ops ⑥ 日常运营**巡检入口 |
| v1.10 | 2026-07-21 | §五细化：crontab 链路、两层选槽（published∧magnet≥2 → TTL/Rec）、与 Ops ⑥ 边界 |
| v1.11 | 2026-07-21 | §5.4 / crontab：`incremental_publish_worker` 脏页检测 → 增量 bake（可选 deploy） |
| v1.12 | 2026-07-25 | §7.3 / §十：Ops ③ 一键生成流程与失败原因可见；「无可用 items」与缺源分流 |
| v1.13 | 2026-07-25 | 纠正「无可用 items」= 拉源 0 条（非 db seed）；说明加源何时有用/对稀缺无效 |
