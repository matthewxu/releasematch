...
# 发版前全站（首页聚合 + sitemap 重算）
python -m workflow.run generate all

# 同步 Trust 五页 + static（generate all 不含此步）
...
- `generate all` 额外写首页、`sitemap.xml`（D3：≤30 indexable 内容页 + Trust 5 + 首页）
...
# 构建 dist + 同步 Trust/static（不上传 CF）
bash scripts/deploy_cf_pages.sh --prepare-only
...
```bash
python -m workflow.run generate all
# 同步 Trust 五页 + static（generate all 不含此步；deploy 脚本已封装）
bash scripts/deploy_cf_pages.sh --prepare-only
