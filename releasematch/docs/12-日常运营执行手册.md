...
Pipeline 在 Jackett/直连拉源后得到 **0 条种子**，且 Demo 回退只覆盖少数固定槽（如 `tv:1396:s04e06`），故最终 `items=[]` 报错。文案里的「db seed」多为历史措辞，**多数情况不是缺库表 seed**。

| 原因 | 是否靠「加更多源」 | 动作 |
|------|-------------------|------|
| Jackett / Indexer / FlareSolverr 故障 | 否（先修连通） | Ops ⑤ Dashboard TEST；`torrent_sources.run status` |
| 公开源覆盖不足（区域/品类缺口） | **有时有用** | 补 `accounts.local.json` 路由 + VPS indexer 后再 `--force` |
| 冷门 / 成人 / 真稀缺（公开站几乎无种） | **基本没用** | 标 `genuine_scarcity`，勿无限加源重试 |
