...
**Ops UI：** ③ 跟踪表 `detail` 列（悬停看全文）显示 pipeline/generate/speedtest 最新失败原因；⑥ 日常「失败槽样例」含 `error` 文案。若错误为「无可用 items；请先 db seed」→ 先补元数据（pipeline `warm_tmdb` / 台账入库），**不是**只加 Jackett indexer。
...
- **「无可用 items」**：槽位元数据未就绪，先 seed/warm_tmdb，再拉源
