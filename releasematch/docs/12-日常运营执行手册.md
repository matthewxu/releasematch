...
| **跨源分母** | `cross_source_total` 均值 **3.28**（max **6**） | 分子 cross≥2 仍 **3/117** → 待 fuzzy 重算 |
...
| fuzzy 跨源重算 | `python scripts/recompute_cross_source_fuzzy.py --all-published --rescore-after` |
...
```bash
python -m workflow.run generate all
# 同步 Trust 五页 + static（generate all 不含此步；deploy 脚本待并入）
rsync -a portal/trust/ portal/dist/trust/
rsync -a portal/static/ portal/dist/static/
cp portal/404.html portal/410.html portal/dist/

bash scripts/seo_c2_checklist.sh
# 或一键（含 generate）：bash scripts/seo_c2_checklist.sh --prepare
# 或：bash scripts/deploy_cf_pages.sh --prepare-only
```
...
| 步骤 | 状态 | 说明 |
|------|------|------|
| `generate all` + sync Trust/static + seo_c2 | ✅ **本地 13 pass** | 见 §7.2 |
| `bash scripts/deploy_cf_pages.sh` | ⏸ | CF Pages 正式上线仍暂缓（**待改脚本自动 sync**） |
...
| seo_c2 FAIL | 看 FAIL 项；Trust 缺页 → 先 sync `portal/trust/` | worklogs SEO §六 |
...
| [seo/iterations/2026-07-05-跨源扩展与全站重拉.md](./seo/iterations/2026-07-05-跨源扩展与全站重拉.md) | 跨源 per-indexer · noindex · refetch-all |
