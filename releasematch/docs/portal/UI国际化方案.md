...
**切换模式**（`I18N_ENABLED=true`）：额外生成 `rm-i18n-data.dynamic`，供 `site.js` 在切换时更新 DOM。

### 4.3 前端切换 — `portal/static/js/site.js`
...
4. 切换后调用 `initResponsiveTables()`，刷新移动端 `data-label`
...
workflow.run generate all
  ...
  ├─ write_all_show_hubs()
  ├─ write_trust_pages()   → portal/dist/trust/*/index.html
  └─ write_sitemap()
```

Trust 五页不再维护 `portal/trust/*.html` 手写副本为主路径；以 **`render_trust.py` + `trust_content.py`** 为准，随 `generate all` 写入 `dist/trust/`。
...
| `portal/generator/templates/partials/i18n_script.html` | 注入 `rm-i18n-data` |
...
- Trust：`/trust/about/` 正文随切换变化

### Trust 与 sitemap

`generate all` 后确认：

- `portal/dist/trust/{about,contact,dmca,privacy,how-matching-works}/index.html` 存在
