...
(function (global) {
  ...
  const HELP_CATALOG = {
    ...
    seo: {
      ...
      api: "POST /api/actions/seo/start → GET …/seo/progress（分项轮询）",
      flow: [
        "后台 run_checks §6.1→6.2→6.3",
        "每完成一项更新 check_id / status / detail",
        "回写批次步骤 seo_c2；重复点击附着轮询",
      ],
      scripts: [
        "workflow/ops/seo_c2_service.py",
        "scripts/seo_c2_checklist.py",
        "workflow/ops/actions.py → run_seo_c2()（同步兼容）",
      ],
      commands: [
        "POST /api/actions/seo/start",
        "GET /api/actions/seo/progress",
        "python scripts/seo_c2_checklist.py",
      ],
      dataFlow: "portal/dist → CheckItem[] → Ops 进度表 → ops_track seo_c2",
      storage: ["批次步骤状态：ops_track_batches", "进度明细见顶部进度条下方表"],
      troubleshoot: [
        "看不到分项 → 硬刷新 Ops；确认走 /seo/start",
        "找不到脚本 → 确认 scripts/seo_c2_checklist.py",
        "检查失败 → 读分项 detail，对照修 dist/元数据",
      ],
    },

    deploy: {
      title: "执行 Deploy",
      api: "POST /api/actions/deploy/start → GET …/deploy/progress",
      flow: [
        "full：逐页 generate all（进度条显示 page_id）→ home/hub/sitemap",
        "incremental：选中槽 bake",
        "upload_only：跳过 prepare",
        "upload=true：流式 wrangler deploy（日志尾）",
        "未勾选正式上传 → 只改本地 dist，公网不变",
      ],
      scripts: [
        "workflow/ops/deploy_flow_service.py",
        "portal/generator/generate_one.py",
        "wrangler deploy",
      ],
      commands: [
        "POST /api/actions/deploy/start",
        "GET /api/actions/deploy/progress",
      ],
      dataFlow: "跟踪批 → portal/dist →（可选）Cloudflare Pages",
      storage: [
        "本地：portal/dist/",
      ...
      troubleshoot: [
        "无分步进度 → 硬刷新；确认走 /deploy/start",
        "本地与公网不一致 → 是否勾选「正式 wrangler 上传」",
        "全量很久 → 正常；看 page_index / page_total",
        "wrangler 失败 → 看进度区日志尾 / CLOUDFLARE_API_TOKEN",
      ],
    },
